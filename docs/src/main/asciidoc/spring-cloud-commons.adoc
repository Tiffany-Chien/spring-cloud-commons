...
WARNING: Although the basic, non-cached, implementation is useful for prototyping and testing, it's much less efficient than the cached versions, so we recommend always using the cached version in production. If the cache is already done by the `DiscoveryClient` Implementation, for example `EurekaDiscoveryClient`, then the load-balancer caching should be disabled to prevent double caching.
